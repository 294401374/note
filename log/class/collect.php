...
class Collect{
	...
	function add($url,$userid,$type,$title,$id,$favorite){
		//收藏		
		global $_SGLOBAL,$_TIDE;
		...
		if($ifcollect==0){
			...
			//插入收藏表
			$time = time();
			$result = $_SGLOBAL['db']->query("insert into ".$_TIDE['tide_collect']." (Title,item_gid,userid,url,CreateDate,Active,Status,favorite)values('".$title."',".$id.",".$userid.",'".$url."',".time().",1,0,".$favorite.")");
			if($result){
				$sql_u_f="update channel_favorite set number=number+1 where id=".$favorite;
